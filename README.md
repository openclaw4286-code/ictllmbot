...
| `LIQUIDATION_SAFETY_BUFFER` | 1.0 | 청산 ≥ SL 거리 (SL만 안 넘으면 됨) |
...
**의미**: 청산가가 SL 거리만큼 (또는 그 이상) 멀도록. SL이 먼저 터치되게 보장.

| SL 거리 | 이상적 L | 실제 L (상한 50) | 청산 거리 |
|---|---|---|---|
| 0.5% | 200 | **50x** | 2.0% (SL의 4배) |
| 1% | 100 | **50x** | 2.0% (SL의 2배) |
| 2% | 50 | **50x** | 2.0% (SL과 동일) |
| 3% | 33.3 | **33x** | 3.0% (SL과 동일) |
| 5% | 20 | **20x** | 5.0% (SL과 동일) |

**포지션 사이즈 공식** (잔고에 따른 자동 계산):
```
리스크 금액 = total equity × 2%  (= 잔고 $92 → $1.84)
notional    = 리스크 / SL%        (= $1.84 / 2% = $92)
margin      = notional / leverage (= $92 / 50 = $1.84)
```

**모든 SL 거리에서 SL 터치 시 손실은 전재산의 2%로 동일.**
